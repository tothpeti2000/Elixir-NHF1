defmodule Nhf1 do
  ...
  defp find_solutions(tree_tent_map, _tents_count_rows, _tents_count_cols, directions)
  ...
  defp find_solutions(tree_tent_map, tents_count_rows, tents_count_cols, tents_so_far) do
    ...
    [first_tree | other_trees] = Map.keys(tree_tent_map)

    if length(Map.get(tree_tent_map, first_tree)) === 0 do
      nil
    else
      [first_tent | other_tents] = Map.get(tree_tent_map, first_tree)

      if violates_row_constraint?(first_tent, tents_so_far, tents_count_rows) ||
           violates_col_constraint?(first_tent, tents_so_far, tents_count_cols) ||
           touches_other_tent?(first_tent, tents_so_far) do
        map = %{tree_tent_map | first_tree => other_tents}

        result = find_solutions(map, tents_count_rows, tents_count_cols, tents_so_far)

        if result === nil, do: tents_so_far, else: result
      else
        map = Map.delete(tree_tent_map, first_tree)

        result =
          find_solutions(map, tents_count_rows, tents_count_cols, tents_so_far ++ [first_tent])
  ...
  defp violates_row_constraint?({i, _j}, tents_so_far, tents_count_rows) do
    ...
    row_tents_so_far = tents_so_far |> Enum.filter(fn {a, _b} -> a === i end)
  ...
  defp violates_col_constraint?({_i, j}, tents_so_far, tents_count_cols) do
    ...
    col_tents_so_far = tents_so_far |> Enum.filter(fn {_a, b} -> b === j end)

    expected_col_count >= 0 && length(col_tents_so_far) + 1 > expected_col_count
  end

  defp touches_other_tent?({i, j}, tents_so_far) do
    Enum.any?(tents_so_far, fn {a, b} -> abs(a - i) <= 1 && abs(b - j) <= 1 end)
  end
end
